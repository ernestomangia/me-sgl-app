using System.ComponentModel.DataAnnotations.Schema;
using System.Data.Entity.Infrastructure.Annotations;
using System.Data.Entity.ModelConfiguration;

using ME.Libros.Dominio.General;

namespace ME.Libros.EF.Mapeos
{
    public class GastoCobradorTypeConfiguration : EntityTypeConfiguration<GastoCobradorDominio>
    {
        public GastoCobradorTypeConfiguration()
        {
            // PK
            HasKey(c => c.Id);

            // Properties
            Property(c => c.Id).HasDatabaseGeneratedOption(DatabaseGeneratedOption.Identity);
            Property(c => c.FechaAlta).IsRequired();

            Property(c => c.FechaGasto).IsRequired();
            Property(c => c.Monto).IsRequired();

            // FK
            HasRequired(c => c.Cobrador);
            HasRequired(c => c.Gasto);

            // Map Table
            ToTable("GastoCobradorDominio");
        }
    }
}
