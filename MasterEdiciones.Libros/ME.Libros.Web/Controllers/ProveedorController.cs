    ...
    public class ProveedorController : BaseController<ProveedorDominio>
    {
        ...
        public JsonResult Eliminar(int id, string redirectUrl)
        {
            ...
            try
            {
                using (ProveedorService)
                {
                    var proveedorDominio = ProveedorService.GetPorId(id);
                    ProveedorService.Eliminar(proveedorDominio);

                    if (isRedirect)
                    {
                        TempData["Id"] = proveedorDominio.Id;
                        TempData["Mensaje"] = string.Format(Messages.EntidadEliminada, Messages.ElProveedor, proveedorDominio.Id);
                    }
                }
            }
            catch (DbUpdateException ex)
